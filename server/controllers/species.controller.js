...
const searchSpecies = async (req, res) => {
  try {
    ...
    // existing options for family, genus, iucnstatus, group, and residency fields
    ...
    let filter = {
      englishName: { $regex: englishname, $options: "i" },
      species: { $regex: searchspecies, $options: "i" },
      scientificName: { $regex: searchscientific, $options: "i" },
    };

    if (startsWith) {
      filter.englishName = { $regex: `^${startsWith}`, $options: "i" };
    }
    ...
    res.status(200).json({ species, total });
  } catch (error) {
    console.log(error);
